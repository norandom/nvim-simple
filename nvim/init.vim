...
function! SetupQuickUI()
    try
        ...
        " Ignore errors if quickui is not loaded
    endtry
endfunction
...
function! OpenMenu()
    try
        ...
        echohl ErrorMsg
        echo "Menu not available (plugin may not be installed)"
        echohl None
    endtry
endfunction

" Enable menu keybindings (including from terminal mode)
noremap <silent><F10> :call OpenMenu()<CR>
inoremap <silent><F10> <Esc>:call OpenMenu()<CR>
tnoremap <silent><F10> <C-\><C-n>:call OpenMenu()<CR>
noremap <space><space> :call OpenMenu()<CR>
tnoremap <space><space> <C-\><C-n>:call OpenMenu()<CR>
